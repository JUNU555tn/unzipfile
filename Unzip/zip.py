...
# Store active tasks to ensure only one process at a time per user
active_tasks = {}
...
async def handle_file(client, message):
    ...
    if user_id in active_tasks:
        await message.reply("❌ You are already processing a file. Please wait until the current task is completed.")
        return
    ...
    if document.mime_type == 'application/zip':
        ...
        try:
            ...
            # Mark the task as active for this user
            ...
            # Remove the task from the active tasks once done
